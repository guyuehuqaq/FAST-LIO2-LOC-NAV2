...
float turn_on_robot::Odom_Trans(uint8_t Data_High, uint8_t Data_Low)
{
  ...
  // The speed unit is changed from mm/s to m/s //速度单位从mm/s转换为m/s
  return data_return;
}
...
void turn_on_robot::Publish_Voltage()
{
  ...
  //Define the data type of the power supply voltage publishing topic //定义电源电压发布话题的数据类型
...
//服务
void turn_on_robot::Set_Charge_Callback(const shared_ptr<turtlesim::srv::Spawn::Request> req,
...
bool turn_on_robot::Get_Sensor_Data_New()
{
  ...
  //Temporary variable to save the data of the lower machine //临时变量，保存下位机数据
  ...
  //Read the data sent by the lower computer through the serial port //通过串口读取下位机发送过来的数据
    ...
    if (Receive_Data.Frame_Tail == FRAME_TAIL) //Verify the frame tail of the packet //验证数据包的帧尾
    {
      ...
      if (error == 0)
      {
        ...
        */
        ...
        //Get the speed of the moving chassis in the Z direction //获取运动底盘Z方向速度
  ...
  while (rclcpp::ok())
  {
    try
    {
      ...
      if (check_AutoCharge_data)
      {
        ...
        Publish_RED();
        //Pub the topic whether the robot finds the infrared signal (charging station) //发布机器人是否寻找到红外信号(充电桩)的话题
...
turn_on_robot::turn_on_robot() : rclcpp::Node("wheeltec_robot")
{
  ...
  //Communicate baud rate 115200 to the lower machine //和下位机通信波特率115200
  this->get_parameter("usart_port_name", usart_port_name); //Fixed serial port number //固定串口号
  this->get_parameter("odom_frame_id", odom_frame_id);
  //The odometer topic corresponds to the parent TF coordinate //里程计话题对应父TF坐标
  this->get_parameter("robot_frame_id", robot_frame_id);
  //The odometer topic corresponds to sub-TF coordinates //里程计话题对应子TF坐标
  ...
  //Create the odometer topic publisher //创建里程计话题发布者
  imu_publisher = create_publisher<sensor_msgs::msg::Imu>("imu/data_raw", 2);
  //Create an IMU topic publisher //创建IMU话题发布者
  voltage_publisher = create_publisher<std_msgs::msg::Float32>("PowerVoltage", 1);
  //Create a battery-voltage topic publisher //创建电池电压话题发布者
  ...
  //回充服务提供
  SetCharge_Service = this->create_service<turtlesim::srv::Spawn>("/set_charge",
                                                                  std::bind(&turn_on_robot::Set_Charge_Callback, this,
                                                                            std::placeholders::_1,
                                                                            std::placeholders::_2));
  ...
  //底盘安全防护关闭话题订阅
  Security_Sub = create_subscription<std_msgs::msg::Int8>("chassis_security", 2,
                                                          std::bind(&turn_on_robot::Security_Callback, this,
                                                                    std::placeholders::_1));
